...
var SuggestionPopup = class extends import_obsidian3.EditorSuggest {
  ...
  setTabInsertionEnabled(val) {
    let self = this;
    this.disableTabInsertion();
    if (val) {
      this.tabKeybindRegistration = self.scope.register([], "Tab", (event) => {
...
var CompletrSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Minimum word trigger length").setDesc("The minimum length a word has to be, to trigger suggestions. This value is ignored by the LaTeX provider.").addText((text) => {
      text.inputEl.type = "number";
    ...
    new import_obsidian4.Setting(containerEl).setName("Tab key for completion insertion").setDesc("This option enables the use of the Tab key alongside of the Enter key to insert suggestions. This will disable the default Tab behavior while the suggestion popup is open. Hold shift to bypass this.").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.enableTabKeyForCompletionInsertion).onChange((val) => __async(this, null, function* () {
  ...
  constructor() {
    ...
    this.handleKeydown = (event, cm) => {
      if (!["Enter", "Tab"].contains(event.key) || event.code === "completr")
        return;
      const view = cm.state.field(import_obsidian5.editorViewField, false);
      if (!view)
        return;
